...
import scipy
from scipy.optimize import root_scalar, brentq, minimize, ridder, fsolve, broyden1
...
def graphMaxMinZeroDrawShow(phi1, phi2=0):
    ...
    # Нуль функції-значення аргумента(х), при якому значення функції(у) дорівнює нулю
        ...
        def intensity_function(frequency):
            return np.sin(2 * np.pi * frequency * t + phi)

        # Функція для пошуку нулів за допомогою brentq
        def find_zeros(f_min, f_max, step):
            zero_points = []
            frequencies = np.arange(f_min, f_max, step_zero)
            for i in range(1, len(frequencies)):
                # Перевірка, чи функція змінює знак на інтервалі [frequencies[i-1], frequencies[i]]
                if intensity_function(frequencies[i - 1]) * intensity_function(frequencies[i]) < 0:
                    zero = fsolve(intensity_function, frequencies[i - 1], frequencies[i])
                    zero_points.append(zero)
            return zero_points

        # Пошук нулів
        zeros = find_zeros(f_min, f_max, step)
        print("Знайдені нулі:", zeros)
